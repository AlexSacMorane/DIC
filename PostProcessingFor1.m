...
%% Put the matlab data
FolderName = '31-07-durham-sand-f24/';
SP = 75;
s = load(strcat('/Volumes/Temp_Storage/RunTest/',...
    FolderName,... %Folder name
    '31JUL2019_2_f24_Durham_2mm_min_40Mpsi_30sec_SP',int2str(SP),'.mat')); % File name 
...
%% You have to change this following the test

te = size(s.Displacement.Times,1)-10;
ts = te-24;
...
% width1xL = zeros(1,12);% zeros(1,size(Times,1)-TimeStep);
% width1yL = zeros(1,12);% zeros(1,size(Times,1)-TimeStep);
% realWidth1L = zeros(1,12);% zeros(1,size(Times,1)-TimeStep);
% angle1L = zeros(1,12);% zeros(1,size(Times,1)-TimeStep);
% 
% width2xL = zeros(1,12);% zeros(1,size(Times,1)-TimeStep);
% width2yL = zeros(1,12);% zeros(1,size(Times,1)-TimeStep);
% realWidth2L = zeros(1,12);% zeros(1,size(Times,1)-TimeStep);
% angle2L= zeros(1,12);% zeros(1,size(Times,1)-TimeStep);
% 
% shearBandNumL = zeros(1,12);% zeros(1,size(Times,1)-TimeStep); 
...
%fSBRecognition = figure('Name','Evolution of SB Recognition','Color','White');
...
%% Shear band recognition  
%[width1x,width1y,realWidth1,angle1,width2x,width2y,realWidth2,angle2,shearBandNum,BW1] = ...
    %ShearBandRecognition(e11,discriminant);

%% Cut to interpolate a Gaussienne
...
save(strcat('/Volumes/Temp_Storage/RunTest/',FolderName,'data/',NameData,'_SP',int2str(SP),'.mat'),'xDataXs','yDataXs','xDataYs','yDataYs')    

end

NameDate = 'SlideDefinition' ; 
save(strcat('/Volumes/Temp_Storage/RunTest/',FolderName,'data/',NameData, '_SP',int2str(SP),'.mat'),'IndiceCutX','IndiceCutY')

%% Saving on an excel

Times = ts:te;

ListWidthX_Slide1 = ListWidthX(1,:)';
ListWidthX_Slide2 = ListWidthX(2,:)';
ListWidthX_Slide3 = ListWidthX(3,:)';
ListWidthX_SlideAve = ListWidthX(4,:)';

ListWidthY_Slide1 = ListWidthY(1,:)';
ListWidthY_Slide2 = ListWidthY(2,:)';
ListWidthY_Slide3 = ListWidthY(3,:)';
ListWidthY_SlideAve = ListWidthY(4,:)';

T = table(Times',ListWidthX_Slide1,ListWidthX_Slide2,ListWidthX_Slide3,...
          ListWidthX_SlideAve,ListWidthY_Slide1,ListWidthY_Slide2,...
          ListWidthY_Slide3,ListWidthY_SlideAve);
      
filename = strcat(FolderName,'20AUG2019_1_f20_Durham_2mm_min_40Mpsi_30sec','_WidthSB_SP',int2str(SP),'.xlsx');
% writetable(T,strcat('/Volumes/Temp_Storage/RunTest/',filename));
